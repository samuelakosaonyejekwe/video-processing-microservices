...
class AuthEventConsumer:
    ...
    def __init__(self):
        ...
        self.auth_events_queue = os.getenv(
            "AUTH_EVENTS_QUEUE"
        )

        self.auth_retry_queue = os.getenv(
            "AUTH_RETRY_QUEUE"
        )
    ...
    def connect(self):
        ...
        while True:
            ...
            try:
                ...
                logger.info(
                    "Connecting auth consumer to RabbitMQ..."
                )
                ...
                logger.info(
                    "Auth RabbitMQ consumer connected successfully"
                )
    ...
    def reconnect(self):
        ...
        logger.warning(
            "Reconnecting auth RabbitMQ consumer..."
        )
    ...
    def process_message(
        ...
        try:
            ...
            if event_type == "user_registered":
                ...
                logger.info(
                    "Processing password reset event"
                )

            elif event_type == "token_revoked":

                logger.info(
                    "Processing token revocation event"
                )

            elif event_type == "suspicious_login_detected":

                logger.warning(
                    "Processing suspicious login event"
                )

            else:

                logger.warning(
                    "Unknown auth event type received: %s",
                    event_type
                )
            ...
            logger.error(
                "Auth consumer processing failed: %s",
                str(error)
            )
    ...
    def start(self):
        ...
        while True:
            ...
            try:
                ...
                logger.info(
                    "Auth consumer waiting for events..."
                )
    ...
    def close(self):
        ...
        try:
            ...
            logger.info(
                "Auth RabbitMQ consumer connection closed"
            )
